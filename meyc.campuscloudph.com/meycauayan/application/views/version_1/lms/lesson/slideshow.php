...
<body style="margin-top: 10px">
	...
	<div class="row row-height">
		<div class="col-md-5 col-sm-12 col-xs-12 left" id="holder">
			<div class="col-xs-12">
				<a href="<?php echo $general_class->ben_link('lms/lesson/index'); ?>"><button class="btn btn-success form-control">Done</button></a>
				<h3>Topic</h3>
				<input type="text" id="lesson_name" name="" class="form-control" value="<?php print_r($views_data['lesson_data']['lesson_name'])?>">
				<h3 style="display: inline;">Objective : </h3>
				<textarea class="form-control" id="objective" style="display: inline;"><?php if($views_data['lesson_data']['objective']): ?><?php echo trim($views_data['lesson_data']['objective']); ?><?php endif; ?></textarea>
				<center >
					<h3>Learning Plan</h3>
					<div class="col-lg-12" style="border-bottom: 2px solid black;"></div>
					<a class="col-lg-6" href="" data-toggle="modal" data-target="#upload_learning_plan_modal"><button class="btn btn-warning form-control" style="margin:20px 0px">Upload</button></a>
					<a class="col-lg-6" href="" data-toggle="modal" data-target="#show_learning_plan_modal"><button class="btn btn-primary form-control" style="margin:20px 0px">View</button></a>
				...
				<?php foreach($data['lesson_folders'] as $folder_key=>$folder_value): ?>
					...
					<div class="col-xs-3 folder">
							...
							<div class="folder_contents">
								<button type="button" class="btn btn-primary col-lg-12 modal_trigger" folder="<?php echo $folder_number; ?>" data-toggle="modal" data-target="#<?php echo str_replace(' ', '_', $folder_value)?>_modal">Upload</button>
		...
		<div class="preview_side col-md-7 col-xs-12 right">
			...
			<div class="col-lg-12">
				...
				<div class="preview_container" style="display: none;">
					...
					<div class="notes_container">
						<h1>Notes</h1>
						<textarea class="notes form-control" file_id=""></textarea>
					</div>
				</div>
	...
	<li id="render_clone" lesson_id="<?php echo $views_data['lesson_id']?>" file_id="" folder="" file_order="" class="file-preview-frame ben-default ui-state-default kv-preview-thumb" data-fileindex="0" data-template="image" style="display: none;">
		<div class="kv-file-content" style="margin-bottom:10px">
			<div class="icon_file video_file"><i class="glyphicon glyphicon-play" style="font-size: 80px;color: #ff6b6b;"></i>Video</div>
			<div class="icon_file image_file"><i class="glyphicon glyphicon-picture" style="font-size: 80px;color:#ed9d2a;"></i>Image</div>
			<div class="icon_file pdf_file"><i class="glyphicon glyphicon-file" style="font-size: 80px;color: #2b669a;"></i>PDF</div>
			<div class="icon_file audio_file"><i class="glyphicon glyphicon-music" style="font-size: 80px;color:#0fcc38;"></i>Audio</div>
			<div class="icon_file powerpoint_file"><img src="http://localhost/sanisidro/resources/datatables/ppt.png" style="width: 100%;height: 84px;">Powerpoint</div>
			<div class="icon_file word_file"><img src="http://localhost/sanisidro/resources/datatables/word.png" style="width: 100%;height: 84px;">Word</div>
		...
		<div class="file-thumbnail-footer">
			<div class="file-actions">
			    <input type="text" value="" name="" lesson_id="<?php echo $views_data['lesson_id']?>" file_id="" class="thumbnail_text form-control" placeholder="(File Name Here)" />
			    <div class="file-footer-buttons">
			        ...
			        <button type="button" style="width: 100%" file_id="" class="delete_button kv-file-remove btn btn-kv btn-default btn-outline-secondary"><i class="glyphicon glyphicon-trash"></i></button>
...
<script type="text/javascript">
	...
	$(".upload").fileinput({
    	...
    	$(document).find("button[title='Remove file']").remove();
    });
		...
		if(confirm("Are you sure you want to delete this file?")){
			...
			$.ajax({
		      	...
		      	success: function (result) {
		           	...
		           	if(result!=""){
		           		$(document).find("li[file_id='"+result+"']").remove();
		           		alert("Successfully Deleted!");
		           	}else{
		           		alert("A little problem was encountered");
		           	}
		      	}
			});
		}else{
